...
const Card = ({item}) => {
  ...
  return (
    <View style={tw`flex-1 px-4 py-4 mb-4`}>
      <Image 
      ...
      <View style={tw`px-2 my-2`}>
        ...
        <View style={tw`flex-row justify-between items-center my-2`}>
          <Text style={tw`text-xs my-1 text-gray-700`}>{item.author}</Text>
          <Text style={tw`text-xs text-gray-700`}>{publishedDate}</Text>
        </View>
        <TouchableOpacity style={tw`bg-redprimary px-4 py-1.5 mt-2 w-32 justify-around items-center rounded-md flex-row`}>
          <Text style={tw`mr-1 text-white text-xs font-semibold`}>Read More...</Text>
          <ArrowRightIcon style={tw`mr-1`} color="#fff" size={18} />
        </TouchableOpacity>
      </View>
      <View style={tw`absolute top-4 right-4 bg-redprimary px-4`}>
        <Text style={tw`text-xs text-white py-1 rounded-md`}>{item.source.name}</Text>
      </View>
    </View>
  )
}
